...
This app replaces those tables with a lightweight, real-time digital calculation that runs entirely in the browser — even offline.
...
A client-side progressive web app that calculates solar azimuth and shadow direction for any location and time on Earth. Designed as a training aid for personnel practicing sun-compass navigation techniques under terrestrial conditions. Works fully offline as an installable PWA.
...
|---|---|---|
| **Algorithm Basis** | Jean Meeus, *Astronomical Algorithms* (simplified) | NREL SPA (Reda & Andreas, 2004), full C port |
| **Azimuth Accuracy** | ~0.08° | ±0.0003° (~1 arc-second) |
| **Minified Size** | ~7 KB (~2 KB gzipped) | ~10 KB (~3 KB gzipped) |
| **Dependencies** | None | None |
| **License** | BSD-2-Clause | MIT (wrapper) / NREL government terms |
| **Source** | Community | U.S. Dept. of Energy — National Renewable Energy Laboratory |

### Decision: nrel-spa

nrel-spa was selected because it is a direct JavaScript port of the NREL Solar Position Algorithm — a product of the U.S. Department of Energy's National Renewable Energy Laboratory. The project lead's directive was clear: government-sourced math is preferred for a tool intended for government training use.

The library implements the full SPA algorithm published by Reda & Andreas (2004) with observatory-grade accuracy (±0.0003°). Standard atmospheric refraction (1013.25 hPa, 15°C) is included for realistic altitude calculations near the horizon. The library is vendored directly into the repository with full license attribution — no npm runtime dependency.
...
*Portfolio capstone demonstrating research-driven software design, astronomical computation, accessibility engineering, and iterative project planning to solve a real-world problem.*
